...
<form id="todo-form" action="" method="get">
    <div class="card">
        <label for="heading">
            <input type="text" class="heading" placeholder="Title of the task..." name="heading" id="heading">
        </label>
        <label for="addnInfo">
        ...
        <label for="dueDate">
            <input type="date" class="due-date-input" placeholder="Due Date..." name="dueDate" id="dueDate">
        </label>
        <br><br>
        <input type="submit" class="okBtn" value="Add">
    </div>
</form>
...
<!-- List of cards ended -->
<script src="../Javascript/toDoListAdder.js">
</script>
</body>
</html>
...
if ($_SERVER['REQUEST_METHOD'] == "GET" and !empty($_SESSION['login_user'])) {
    ...
    if (!empty($heading)) {
        $query = "INSERT INTO done(username, heading, addnInfo, date) VALUES('$username','$heading','$addnInfo','$date')";
...
if (!empty($_SESSION['login_user'])) {
    $length = 0;
    $username = $_SESSION['login_user'];
    $query_2 = "SELECT * FROM done WHERE username = '$username'";
    ...
    while ($length >= 0) {
        ...
        "<script>
            ...
            deleteForm.action = '../PHP/deleteEntryFromDone.php';
